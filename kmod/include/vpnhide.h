...
#define VPNHIDE_VERSION_CODE 20506
...
/* Versioned snapshot that keeps the v1 ioctl layout available to older
 * helpers while returning hook totals and exact per-port counters together. */
...
/* ABI v4 keeps the variable-length v3 sections, but stores declarative
 * membership modes.  INCLUDE means listed UIDs are targets; EXCLUDE means
 * every eligible UID except the listed UIDs is a target. */
