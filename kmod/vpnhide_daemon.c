...
static const char *nomount_find_tool(void)
{
	...
	if (nomount_tool)
		return nomount_tool;
	for (i = 0; i < sizeof(nomount_tool_candidates) /
			 sizeof(nomount_tool_candidates[0]); i++) {
...
static struct nomount_registered_path *nomount_find_registered_path(const char *path)
{
	size_t i;
...
static void nomount_register_path(const char *path)
{
	...
	susfs_log_error("NoMount registration cache is full; path %s was not cached", path);
}
...
static void file_hiding_sync_interfaces(const struct vpnhide_vpn_ifindexes *vpns)
{
	int i;
